...
class ExpDataset(torch.utils.data.Dataset):
    """
    Fast dataset: converts DataFrame -> single float32 numpy array once,
    then __getitem__ is pure numpy slicing (no pandas .loc).
    """
    def __init__(self, df: pd.DataFrame, gene_exp_input: list, gene_exp_eval: list = None):
        # keep index labels for bookkeeping / saving
        self.index = df.index.to_list()

        # Keep column order and build column->position map
        cols = df.columns.to_list()
        col2pos = {c: i for i, c in enumerate(cols)}

        # Precompute integer column indices (fast)
        self.input_idx = np.array([col2pos[c] for c in gene_exp_input], dtype=np.int64)

        self.eval_idx = None
        if gene_exp_eval is not None and len(gene_exp_eval) > 0:
            self.eval_idx = np.array([col2pos[c] for c in gene_exp_eval], dtype=np.int64)

        # One contiguous matrix (avoid copies if already float32)
    ...
    def __getitem__(self, i):
        row = self.X[i]  # 1D view
        out = {
            "input": row[self.input_idx],
            "index": self.index[i],
        }
        if self.eval_idx is not None:
            out["eval"] = row[self.eval_idx]
        return out
